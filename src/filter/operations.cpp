    ...
    uint8_t* output)
{
    ...
    for (size_t x = 0; x < ncols; ++x) {
        for (size_t y = 0; y < nrows; ++y) {
            ...
            #else
            for (int ky = 0; ky < kernel._nrows; ++ky) {
                for (int kx = 0; kx < kernel._ncols; ++kx) {
            #endif
                    #if OPTIMIZE_1
                    auto kc = *kc_p++;
                    #elif OPTIMIZE_2
                    ...
                    #else
                    int input_x = static_cast<int>(x - kernel._ncols / 2 + kx);
                    int input_y = static_cast<int>(y - kernel._nrows / 2 + ky);
                    #endif
